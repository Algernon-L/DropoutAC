...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action, dropout_rate, hidden_sizes=[256, 256]):
...
class DropoutAC(object):
    def __init__(
        ...
        self.actor1 = Actor(state_dim, action_dim, max_action, dropout_rate, hidden_sizes).to(self.device)
        self.actor1_target = copy.deepcopy(self.actor1)
        self.actor1_optimizer = torch.optim.Adam(self.actor1.parameters(), lr=actor_lr)

        self.actor2 = Actor(state_dim, action_dim, max_action, dropout_rate, hidden_sizes).to(self.device)
    ...
    def train_one_q_and_pi(self, replay_buffer, update_a1=True, batch_size=256):
        ...
        with torch.no_grad():
            self.actor1_target.eval()
            self.actor2_target.eval()
        ...
        if update_a1:
            ...
            critic1_loss = F.mse_loss(current_Q1, target_Q) + self.regularization_weight * F.mse_loss(current_Q1,
                                                                                                      current_Q2)
            ...
            ## critic regularization
            critic2_loss = F.mse_loss(current_Q2, target_Q) + self.regularization_weight * F.mse_loss(current_Q2,
                                                                                                      current_Q1)
